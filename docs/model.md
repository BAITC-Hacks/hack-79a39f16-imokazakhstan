...
### Отдельная проверка прогноза ветра

`evaluation/check_wind.py` повторил тот же ежемесячный expanding backtest для
ветрового ExtraTrees-компонента прежней модели: 48 часов, 33 263 одинаковые пары.
RMSE **3.477819 м/с**, MAE **2.882098 м/с**; историческое среднее RMSE 3.641736 м/с,
сохранение последнего ветра RMSE 4.670762 м/с. Выигрыш относительно среднего около 4.5%,
поэтому это рабочий умеренно точный baseline, не надёжный метеопрогноз.
Итоги по месяцам и суткам горизонта: `models/artifacts/wind-audit/report.json`.
Финальные веса `wind-audit/wind_model.joblib` содержат модель, feature_names,
trained_through, units и source hashes. Загружать joblib только из доверенного источника.
Вход: `np.r_[features(hourly_frame, issue)[0], turbine_index]` для каждой турбины,
где turbine_index равен 0 или 1, локальный issue строго 06:00 UTC+5, frame использует
почасовые END timestamps. Вызов `artifact['model'].predict(feature_matrix)` возвращает
м/с, форма `(number_of_turbines, 48)`. Пример февраля сохранён отдельно и не является
оценкой по факту. Веб-интеграция может показывать этот ветер отдельным рядом;
основной CatBoost мощности не использует его как обязательный промежуточный шаг.
Нужен будущий погодный прогноз для следующей проверяемой гипотезы улучшения;
эффект его добавления ещё не измерен на этих данных.
